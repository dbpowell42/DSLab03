...
Password::~Password()
{
    int all_size = all_words->size();
    int viable_size = viable_words->size();
    for(int i = 1; i <= all_size; i++)
        all_words->remove(i);
    for(int i = 1; i <= viable_size; i++)
...
int Password::getNumMatches(String* curr, String* guess)
{
    int str_length = curr->length();
    int num_matches = 0;

    for(int i = 0; i < str_length; i++)
    {
        if(guess->charAt(i) == curr->charAt(i))
            num_matches++;
    }
    return num_matches;
}
